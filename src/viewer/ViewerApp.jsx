...
export default function ViewerApp() {
  ...
  const [config, setConfig] = useState(null)        // Configuración del evento
  const [textos, setTextos] = useState(null)        // Textos y precios
  const [uuid, setUuid] = useState(null)            // UUID del evento
  const [photos, setPhotos] = useState([])          // Fotos del evento
  const [currentPage, setCurrentPage] = useState(1) // Página actual
  const [lastPage, setLastPage] = useState(1)       // Última página
  const [printCount, setPrintCount] = useState(0)   // Contador de impresiones
  const [error, setError] = useState(null)          // Mensaje de error

  // --- Estados de modales ---
  const [showEventModal, setShowEventModal] = useState(false)  // Modal para ingresar evento
  const [eventInput, setEventInput] = useState('')             // Input del código de evento
  const [eventError, setEventError] = useState('')             // Error en el código
  const inputRef = useRef(null)                                // Referencia para auto-focus

  // --- Estados de impresión ---
  const [selectedPhoto, setSelectedPhoto] = useState(null)     // Foto seleccionada para imprimir
  const [copies, setCopies] = useState(1)                      // Número de copias (1, 2 o 3)
  const [printing, setPrinting] = useState(false)              // Indicador de impresión en curso
  const [printDone, setPrintDone] = useState(false)            // Indicador de impresión completada
  ...
  /**
   * Cargar configuración inicial y contador de impresiones
   */
  useEffect(() => {
    Promise.all([
  ...
  /**
   * Conectar al evento cuando cambia la configuración
   */
  useEffect(() => {
  ...
  /**
   * Cargar fotos del evento actual
   */
  ...
  // Cargar fotos cuando cambia el UUID o la página
  useEffect(() => {
    loadPhotos()
  }, [loadPhotos])

  // Recargar fotos automáticamente cada X segundos (config.timer)
  ...
  /**
   * Abrir modal de impresión para una foto
  ...
  async function handlePrint() {
    ...
    try {
      // Imprimir según el número de copias
  ...
  /**
   * Obtener el precio según el número de copias
   * @param {number} n - Número de copias (1, 2 o 3)
   * @returns {string|null} - Precio o null si no está configurado
   */
  ...
  return (
    <div className="viewer-app d-flex flex-column bg-dark text-light" style={{ height: "100vh", overflow: "hidden" }}>
      ...
      {showEventModal && (
        <div
          className="modal d-block event-modal-overlay"
          tabIndex="-1"
        >
            ...
            <div className="modal-content bg-dark border border-secondary event-modal">
              <div className="modal-body text-center p-4 p-md-5">
                ...
                <div className="input-group mb-2 event-modal-input-group">
                  <span className="input-group-text bg-black border-secondary text-warning fw-bold font-mono fs-5">
                    ev-
                  </span>
                  <input
                    ref={inputRef}
                  ...
                  <div className="col-md-5 print-panel">
                    ...
                    <div>
                      ...
                      <div className="copies-buttons">
                        {[1, 2, 3].map(n => (
                          <button
                            key={n}
                            className={`btn fw-bold copy-btn ${
                              copies === n ? 'copy-btn-active' : 'copy-btn-inactive'
                            }`}
                            onClick={() => setCopies(n)}
                          >
                            <span>
                              {copies === n ? (
                                <i className="bi bi-check-circle-fill copy-icon" />
                              ) : (
                                <i className="bi bi-circle copy-icon" />
                              )}
                              {n} {n === 1 ? 'copia' : 'copias'}
                            </span>
                            {textos?.[`precio${n}`] && (
                              <span
                                className={`badge ${
                                  copies === n ? 'copy-price-badge-active' : 'copy-price-badge-inactive'
                                }`}
                              >
                                {textos[`precio${n}`]}€
                              </span>
                    ...
                    <div className="print-actions">
                      {printDone ? (
                        ...
                        <>
                          <button
                            className="btn btn-warning text-dark fw-bold print-button"
                            onClick={handlePrint}
                            disabled={printing}
                          >
                            {printing ? (
      ...
      <header className="viewer-header">
        {error && (
          <div className="alert alert-danger viewer-error-alert">
            {error}
          </div>
        )}
      </header>
        ...
        {!uuid ? (
          ...
          <div className="viewer-gallery-empty">
            ...
            <p className="empty-text">
              Esperando fotos de <strong>{config?.evento}</strong>…
            </p>
          </div>
        ) : (
        ...
        <nav className="viewer-pagination">
          <ul className="pagination pagination-sm mb-0">
            ...
            {Array.from({ length: lastPage }, (_, i) => i + 1).map(page => (
              <li
                key={page}
                className={`page-item ${page === currentPage ? 'active pagination-active' : ''}`}
              >
                <button
                  className="page-link pagination-btn"
      ...
      <footer className="viewer-footer">
        <div className="viewer-footer-content">
          ...
          <span className="print-count">
            ...
            <span className="print-count-number">{printCount}</span> impresiones
          </span>

          <button
            className="btn btn-warning change-event-btn"
            onClick={() => {
              setEventInput('')
              setEventError('')
              setShowEventModal(true)
...
/**
 * Tarjeta individual de foto para la galería
 * @param {Object} props
 * @param {Object} props.photo - Datos de la foto
 * @param {Function} props.onSelect - Función al seleccionar la foto
 */
